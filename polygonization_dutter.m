...
Level3=0; % U-shape
% It should be noted that level 3 can be independent of level 2.
% It means that a shape can be passed through level 1 and then to level 3.
...
if Level2
    ...
    dist_si_ti=zeros(1,4);
    for i=1:4
        dist_si_ti(i)=dist(P(S1(i),:),M1(:,i)');
    end
    ...
    if isempty(idx)
        ...
        for j=1:length(idx) 
            ...
            d_backward=zeros(1,length(Psub1{B(1)}));
            for i=1:length(Psub1{B(1)})
                d_backward(i)=dist_to_line(Psub1{B(1)}(i,:),[M1(:,B(1)) M1(:,B(2))]); 
            end
            idx_sb=max(find(d_backward<2)); % the constant 2 meters chosen by Dutter
            sb(j,:)=Psub1{B(1)}(idx_sb,:);

            % Forward split-point (sf) - See Equation (3.4) page 55
            d_forward=zeros(1,length(Psub1{F(1)}));
            for i=1:length(Psub1{F(1)})
                d_forward(i)=dist_to_line(Psub1{F(1)}(i,:),[M1(:,F(1)) M1(:,F(2))]);
            end
            idx_sf=min(find(d_forward<2)); % the constant 2 meters chosen by Dutter
            sf(j,:)=Psub1{F(1)}(idx_sf,:);

            % Rotate sb and sf with origin of t(i) with the positive x-axis 
            % going through t(i-1) and positive y-axis through t(i+1)            
            if idx(j)==1
                h(j,:)=parallel_intersection(sf(j,:)',[M1(:,4) M1(:,idx(j))],...
                                                 sb(j,:)',[M1(:,idx(j)) M1(:,idx(j)+1)],0);
            else
                h(j,:)=parallel_intersection(sf(j,:)',[M1(:,idx(j)-1) M1(:,idx(j))],...
                                                 sb(j,:)',[M1(:,idx(j)) M1(:,idx(j)+1)],0);
            end
            
            % Middle split-point (sm) - Equation (3.6) page 54
            Ptmp=[Psub1{B(1)}; Psub1{idx(j)}];
    ...
    for i=1:length(idx2)
        ...
        for j=1:n
            ...
            if w==1
                ...
                D_right(j)=dist_to_line(p(j,:),[M0(:,2) M0(:,3)]);
            end
        end
